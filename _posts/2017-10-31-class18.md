...
##        <chr>    <chr> <int>      <chr>
##  1 id_000362    valid     5     crocus
##  2 id_000506    train     7  tigerlily
##  3 id_000778    valid    10  sunflower
##  4 id_000274    train     4   bluebell
##  5 id_000084    train     2   snowdrop
##  6 id_000279    train     4   bluebell
##  7 id_000518    train     7  tigerlily
##  8 id_000670    train     9 fritillary
##  9 id_000511    train     7  tigerlily
## 10 id_000285    valid     4   bluebell
## # ... with 590 more rows
{% endhighlight %}

These images are 64 pixels by 64 pixels. Four times larger than
the thumbnails we used of our data. Class "2" consists of the
flower colt's foot. Let's see at a few of the images to get a
...
not sufficent for distinguishing all classes. Buttercups and windflowers,
...
Columns 3-6 are the ligher greys, which are not very common in 
a dataset of flowers. The 8th column is the orange color and 
the 9th a greenish color, both popular from the flowers and the
background greenery.
...
We see that some expected patterns here. The orange ("#FF9900")
has a high value for buttercups and the white ("#FFFFFF") has a 
large value for colts foot.
...
The reason for these results is that the first elastic net likely
...
set of colors, lets set alpha to 0.2 to spread the weights 
...
in this measurement. Dandelions in particular have a lot of edges.
...
This shows a clearly differentiation of the flowers. Sunflowers, with
the seeds in the middle of the flower, have a lot of edges whereas
the smooth buttercup petals have very little edges in the middle of
the photo.
...
The resulting model performs much better than the color alone.
...
what features would help distinguish the "colts foot" and "daffodil"
flowers from the others as false-positives for these classes are
causing a large portion of the remaining errors.
