...
```{r, message = FALSE, warning = FALSE}
qplot(displ, cty, data = cars)
```

Within this plot, we can mark to classes of cars.

```{r, message = FALSE, warning = FALSE}
qplot(displ, cty, data = cars, color = class, size = I(2)) +
...
partial derivaties to iteratively solve the equation. There are two basic
techniques for doing this, both useful in different situations.

## Coordinate descent

The idea of Coordiante descent is to cycle through the variables,
...
for (i in 2:iter) {
  ...
  grad <- -2 * c(sum(y - alpha_vals[i-1] - x * beta_vals[i-1]),
                 sum(y*x - alpha_vals[i-1]*x - x^2))
  
  # take a step in the direction of the negative gradient
  alpha_vals[i] <- alpha_vals[i-1] + (-rho) * grad[1]
  beta_vals[i] <- beta_vals[i-1] + (-rho) * grad[2]
}
